...
function file_get_html() {
    ...
    $args = func_get_args();
    $s = @call_user_func_array('file_get_contents', $args);
    if ($s != FALSE) $dom->load($s, true);
    else $dom = NULL;
    return $dom;
}
...
class simple_html_dom {
    ...
    function loadFile() {$args = func_get_args(); $s = @call_user_func_array('file_get_contents', $args); if ($s == FALSE) $s = ''; $this->load($s, true); }
}
?>
